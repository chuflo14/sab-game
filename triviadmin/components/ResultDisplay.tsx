...
export default function ResultDisplay({ ticket, store, config }: ResultDisplayProps) {
    ...
    return (
        <div className="flex flex-col items-center justify-center min-h-screen bg-black text-white p-8 text-center animate-in fade-in zoom-in duration-500">
            <h1 className="text-6xl font-black text-green-500 uppercase tracking-tighter mb-4">
                ¡GANASTE!
            </h1>
            ...
            <div className="bg-white p-6 rounded-2xl mb-8 shadow-2xl shadow-green-500/20">
                <QRCode value={ticket.token || ticket.id} size={256} />
                <p className="text-black font-mono font-bold text-2xl mt-4 tracking-widest">{ticket.token}</p>
            </div>

            {store ? (
                <div className="bg-neutral-900 border border-neutral-800 p-6 rounded-xl max-w-lg w-full mb-8">
                    <h2 className="text-xl font-bold text-gray-400 mb-4 uppercase text-sm tracking-wider">Retira en:</h2>
                    <p className="text-3xl font-bold text-white mb-2">{store.name}</p>
                    <p className="text-xl text-gray-400 mb-4">{store.address}</p>

                    <div className="flex gap-4 justify-center">
                        <div className="flex items-center gap-2 text-green-400 bg-green-900/20 px-4 py-2 rounded-full border border-green-900/50">
                            <span>WhatsApp:</span>
                ...
                <div className="bg-red-900/20 border border-red-900/50 p-6 rounded-xl max-w-lg w-full mb-8 text-red-200">
